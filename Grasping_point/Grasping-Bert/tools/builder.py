...
def resume_model(base_model, args, logger = None):
    ...
    if not os.path.exists(ckpt_path):
        ...
        return 0, 0
    print_log(f'[RESUME INFO] Loading modules weights from {ckpt_path}...', logger = logger )

    # load state dict
    map_location = {'cuda:%d' % 0: 'cuda:%d' % args.local_rank}
    state_dict = torch.load(ckpt_path, map_location=map_location)
    # parameter resume of base modules
...
def load_model(base_model, ckpt_path, logger = None):
    ...
    state_dict = torch.load(ckpt_path, map_location='cpu')
    # parameter resume of base modules
    if state_dict.get('modules') is not None:
        base_ckpt = {k.replace("module.", ""): v for k, v in state_dict['modules'].items()}
